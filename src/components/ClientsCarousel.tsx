...
const ClientsCarousel = () => {
  const [topRowClients, setTopRowClients] = useState<{ name: string; logo_url: string }[]>([]);
  const [bottomRowClients, setBottomRowClients] = useState<{ name: string; logo_url: string }[]>([]);
  ...
  useEffect(() => {
    const loadClients = async () => {
      const { data, error } = await supabase
        .from('clients')
        .select('id, name, logo_url, display_order, row_position')
        .eq('active', true)
        .order('display_order', { ascending: true });

      if (error || !data || data.length === 0) {
        // Use fallback logos if no clients in database - split evenly
        const half = Math.ceil(fallbackLogos.length / 2);
        setTopRowClients(fallbackLogos.slice(0, half));
        setBottomRowClients(fallbackLogos.slice(half));
      } else {
        setTopRowClients(data.filter(c => c.row_position === 1));
        setBottomRowClients(data.filter(c => c.row_position === 2));
      }
      setLoading(false);
    };
  ...
  if (topRowClients.length === 0 && bottomRowClients.length === 0) {
    return null;
  }
  ...
  return (
    <section className="w-full py-16 md:py-24 bg-black/50 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4">
        ...
        {/* First Carousel - Top Row - Moving Left */}
        {topRowClients.length > 0 && (
        ...
        {/* Second Carousel - Bottom Row - Moving Right (Reverse) */}
        {bottomRowClients.length > 0 && (
